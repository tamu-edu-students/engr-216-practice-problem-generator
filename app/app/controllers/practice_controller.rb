class PracticeController < ApplicationController
    ...
    def submit_test
      ...
      exam_questions.each do |q|
        ...
        if question_type == "Multiple choice"
          correct_choice, is_correct = evaluate_multiple_choice(answer_choices, submitted_answer)
        else
    ...
    def handle_problem_generation
      if session[:try_another_problem]
        %i[submitted_answer solution question_text question_img question_id try_another_problem is_correct explanation round_decimals question_kind].each do |key|
          session.delete(key)
        end
      end
    ...
    def handle_practice_test_generation
      ...
      @exam_questions = selected_questions.map do |question|
        variable_values = generate_random_values(question.variables, question.variable_ranges, question.variable_decimals)
        formatted_text = question.template_text.present? ? format_template_text(question.template_text, variable_values, question.variable_decimals, question.variables) : question.text
